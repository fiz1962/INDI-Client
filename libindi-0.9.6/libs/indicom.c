...
void tty_error_msg(int err_code, char *err_msg, int err_msg_len)
{
  ...
  switch (err_code)
 {
	...
	case TTY_READ_ERROR:
        sprintf(error_string, 512, "Read Error: %s", strerror(errno));
		strncpy(err_msg, error_string, err_msg_len);
		break;
		
       case TTY_WRITE_ERROR:
        sprintf(error_string, 512, "Write Error: %s", strerror(errno));
		strncpy(err_msg, error_string, err_msg_len);
		break;

	case TTY_SELECT_ERROR:
        sprintf(error_string, 512, "Select Error: %s", strerror(errno));
		strncpy(err_msg, error_string, err_msg_len);
		break;
	...
	case TTY_PORT_FAILURE:
      if (errno == EACCES)
        sprintf(error_string, 512, "Port failure Error: %s. Try adding your user to the dialout group and restart (sudo adduser $username dialout)", strerror(errno));
      else
        sprintf(error_string, 512, "Port failure Error: %s. Check if device is connected to this port.", strerror(errno));
